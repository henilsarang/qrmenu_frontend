...
const ShoppingCart = ({ items, onAdd, onRemove, onPaymentDone,color }) => {
    ...
    const totalPrice = useMemo(
        () => items.map((i) => i.quantity * i.price).reduce((a,b) => a+b,0),[items] 
    );
                    ...
                    {items.map  ((item) => (
                        <div key={item.id} className="d-flex mb-4 align-items-center" >
                            ...
                            <div className="d-flex align-items-center">
                                <OperationButton variant="lightgray" size="sm" onClick = {() => onRemove(item)}> - </OperationButton>
                                <span>{item.quantity}</span>
                                <OperationButton variant="lightgray" size="sm" onClick = {() => onAdd(item)}> + </OperationButton>
                            </div>
                        </div>
                    ))}
                    <hr />
